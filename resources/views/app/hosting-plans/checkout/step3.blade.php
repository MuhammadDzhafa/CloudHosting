<div class="w-full" id="step-3-container">
    ...
    <div class="pricing-section mt-4" id="pricing-section-custom-{{ $id }}" data-id="{{ $id }}">
                ...
                @if ($loop->index < 3)
                    ...
                    <div class="pricing-info flex justify-between items-center">
                        ...
                        <label class="checkbox is-outlined is-circle is-info">
                            <input type="checkbox" name="billing_period" data-custom="true"
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (parsedData?.[0]) {
            document.getElementById('domain-name').value = parsedData.domain_name || '';
            document.getElementById('billing-period').value = parsedData.billing_period || '';
            document.getElementById('price').textContent = parsedData.price || '';
            if (hostingData.ram) {
                document.getElementById('ram').textContent = parsedData.ram;
            }
            if (parsedData.cpu) {
                document.getElementById('cpu').textContent = parsedData.cpu;
            }
            if (parsedData.storage) {
                document.getElementById('storage').textContent = parsedData.storage;
            }
        }
    });
</script>