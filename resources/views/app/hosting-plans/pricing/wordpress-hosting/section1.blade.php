<div class="bg-gradient-custom section-frame padding-1 lg:h-[450px]">

    <div class=" columns">
        <div class="column is-6">
            <h2 class="title-section md:text-center lg:text-left text-white lg:mb-10">
                WordPress Hosting
            </h2>

            <p class="text-base-hero text-primary text-white lg:mb-12">
                Get the best value in WordPress hosting with easy setup, fast loading times, and comprehensive support.
            </p>

            <div class="button-container">
                <a class="button h-button is-outlined rounded-full border-1 px-4 py-3" style="font-family: unset;">
                    <span class="btn-text explore-button" onclick="scrollToSection()">Choose Your Plan</span>
                    <svg class="btn-icon" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M4.16669 10H15.8334M10.8334 5L15.8334 10L10.8334 15" stroke="currentColor"
                            stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </a>
            </div>
        </div>

        <div class="column is-6">
        </div>
    </div>

</div>

<div class="absolute right-36 top-[100px] hidden lg:block">
    <img src="/assets/img/wordpress-logo.svg" alt="" class="w-[400px] h-auto">
</div>