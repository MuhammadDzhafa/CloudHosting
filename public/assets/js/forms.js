        ...
        if (activeTab === 'hosting-only') {
            ...
            switch(window.currentStep) {
                ...
                case 2:
                    ...
                    if (userLogin === 'false') {
                    const selectedPlanElements = document.querySelector('[data-selected="true"]');
                    const selectedBillingElements = document.querySelector('input[name="billing_period"]:checked');

                    if (!selectedPlanElements || !selectedBillingElements) {
                        console.error('No selected plan or billing period found');
                        return;
                    }
                
                    const isCustomPlan = selectedPlanElements.classList.contains('custom-plan');
                
                    // Perbaikan: Ambil data yang lebih spesifik dari view
                    const data = {
                        hosting_plans_id: isCustomPlan 
                            ? selectedPlanElements.getAttribute('data-id') 
                            : selectedPlanElements.getAttribute('data-hosting-plan-id'),

                        name: isCustomPlan 
                            ? 'Custom Hosting Plan' 
                            : selectedPlanElements.querySelector('.plan-title')?.textContent.trim(),

                        plan_type: isCustomPlan ? 'custom' : 'regular',

                        domain_name: document.querySelector('#domain-name')?.value || 'kazee.id',

                        billing_period: selectedBillingElements.value,

                        price: selectedBillingElements.getAttribute('data-price') || 
                               selectedBillingElements.dataset.price || 0,

                        // Perbaikan ekstraksi spesifikasi
                        ram: isCustomPlan
                        ? Array.from(selectedPlanElements.querySelectorAll('.spec-item'))
                            .find(el => el.textContent.includes('RAM'))
                            ?.querySelector('span')
                            ?.textContent.split(' ')[1]
                        : selectedPlanElements.querySelector('.spec-item:nth-child(1) span')?.textContent.split(' ')[1] || 'Default',


                        cpu: isCustomPlan
                          ? Array.from(selectedPlanElements.querySelectorAll('.spec-item'))
                              .find(el => el.textContent.includes('CPU'))
                              ?.querySelector('span')
                              ?.textContent.split(' ')[1]
                          : selectedPlanElements.querySelector('.spec-item:nth-child(2) span')?.textContent.split(' ')[1] || 'Default',
                                            
                        storage: isCustomPlan
                          ? Array.from(selectedPlanElements.querySelectorAll('.spec-item'))
                              .find(el => el.textContent.includes('Storage'))
                              ?.querySelector('span')
                              ?.textContent.split(' ')[1]
                          : selectedPlanElements.querySelector('.spec-item:nth-child(3) span')?.textContent.split(' ')[1] || 'Default',
                                            

                        active_date: new Date().toISOString().split('T')[0],
                        expired_date: calculateExpiredDate(selectedBillingElements.value),
                    };
